...
class kg_purchase_indent(osv.osv):
	...
	_columns = {
		...
		'purpose': fields.selection([('consu', 'Consumables'), ('project', 'Project'), ('asset', 'Asset')], 'Purpose', readonly=True, states={'draft': [('readonly', False)],'in_progress':[('readonly',False)]}),
	...
	def tender_in_progress(self, cr, uid, ids, context=None):
		...
		if not obj.name:
			...
			indent_no = self.pool.get('ir.sequence').get(cr, uid, 'kg.purchase.indent') or ''
			indent_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.purchase.indent')])
			rec = self.pool.get('ir.sequence').browse(cr,uid,indent_no_id[0])
			print "--------------------------->",rec
			cr.execute("""select generatesequenceno(%s,'%s','%s') """%(indent_no_id[0],rec.code,obj.date_start))
