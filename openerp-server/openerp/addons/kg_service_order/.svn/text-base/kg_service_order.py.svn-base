...
class kg_service_order(osv.osv):
	...
	_columns = {
		...
		'product_id': fields.related('service_order_line','product_id', type='many2one', relation='product.product', string='Product'),
	}
	...
	def confirm_order(self, cr, uid, ids,context=None):
		...
		for t in self.browse(cr,uid,ids):
			...
			if not t.name:
				...
				so_no = self.pool.get('ir.sequence').get(cr, uid, 'kg.service.order') or ''
				so_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.service.order')])
				rec = self.pool.get('ir.sequence').browse(cr,uid,so_no_id[0])
				print "--------------------------->",rec
				cr.execute("""select generatesequenceno(%s,'%s','%s') """%(so_no_id[0],rec.code,t.date))
				so_name = cr.fetchone();	
				self.write(cr,uid,ids,{'name':str(so_name[0])})			
