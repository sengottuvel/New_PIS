...
class kg_purchase_invoice_register(report_sxw.rml_parse):
	...
	def get_data(self,form):
		...
		if form['direct_id']:
			...
			for ids1 in form['direct_id']:
				direct.append("pi.id = %s"%(ids1))	
					
		if direct:
			direct = 'and ('+' or '.join(direct)
			direct =  direct+')'
		else:
		...
		if (form['invoice_id'] and not form['voucher_id']) or (form['direct_id']):
			...
			SELECT
				...
				select distinct pi.id AS pi_id,
					...
					0 AS so_id,
					'' AS remark,
					to_char(pi.expense_date,'dd/mm/yyyy') AS inv_date,
					pi.invoice_no AS sup_inv_no,
					to_char(pi.invoice_date,'dd/mm/yyyy') AS sup_inv_date,
					pi.invoice_date AS sup_inv_date1,
					'' AS po_no,
					'' AS po_date,
					'' AS so_no,
					'' AS so_date,
					...
					pi.amount_total as total		   

					FROM  direct_entry_expense pi		  
					JOIN res_partner res ON (res.id=pi.supplier_id)
					where pi.state='approved' and pi.approve_date::date >=%s and pi.approve_date::date <=%s and pi.payment_type = %s ''' + direct + '''
			  '''
			  
			  
			  
			  
			,(form['date_from'],form['date_to'],form['date_from'],form['date_to'],form['date_from'],form['date_to'],form['date_from'],form['date_to'],direct_payment))
		
		elif (form['voucher_id'] and not form['invoice_id']) or (form['direct_id']):
			...
			self.cr.execute('''
				...
				,(form['date_from'],form['date_to'],form['date_from'],form['date_to'],direct_payment))
			...
			self.cr.execute('''
				...
				SELECT
				 ...
				 ,(form['date_from'],form['date_to'],form['date_from'],form['date_to'],form['date_from'],form['date_to'],form['date_from'],form['date_to'],form['date_from'],form['date_to'],direct_payment))
