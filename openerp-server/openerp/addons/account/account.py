...
#				if can_compute:
	...
	def approve_account(self, cr, uid, ids, context=None):
		return self.write(cr, uid, ids, {'state':'approved'})
		
	def edit_account(self, cr, uid, ids, context=None):
...
class account_tax(osv.osv):
	...
	_description = 'Tax'
	_columns = {
		'name': fields.char('Tax Name', size=64, required=True, translate=True, help="This name will be displayed on reports",readonly=True, states={'waiting':[('readonly',False)]}),
		'sequence': fields.integer('Sequence', required=True,readonly=True, states={'waiting':[('readonly',False)]},help="The sequence field is used to order the tax lines from the lowest sequences to the higher ones. The order is important if you have a tax with several tax children. In this case, the evaluation order is important."),
		'amount': fields.float('Amount', required=True, readonly=True, states={'waiting':[('readonly',False)]},digits_compute=get_precision_tax(), help="For taxes of type percentage, enter % ratio between 0-1."),
		'active': fields.boolean('Active', readonly=True, states={'waiting':[('readonly',False)]},help="If the active field is set to False, it will allow you to hide the tax without removing it."),
		'type': fields.selection( [('percent','Percentage'), ('fixed','Fixed Amount'), ('none','None'), ('code','Python Code'), ('balance','Balance')], 'Tax Type', required=True,
			readonly=True, states={'waiting':[('readonly',False)]},help="The computation method for the tax amount."),
		'applicable_type': fields.selection( [('true','Always'), ('code','Given by Python Code')], 'Applicability', required=True,readonly=True, states={'waiting':[('readonly',False)]},
			help="If not applicable (computed through a Python code), the tax won't appear on the invoice."),
		'domain':fields.char('Domain', size=32,readonly=True, states={'waiting':[('readonly',False)]},help="This field is only used if you develop your own module allowing developers to create specific taxes in a custom domain."),
		'account_collected_id':fields.many2one('account.account', 'Invoice Tax Account',readonly=True, states={'waiting':[('readonly',False)]}, help="Set the account that will be set by default on invoice tax lines for invoices. Leave empty to use the expense account."),
		'account_paid_id':fields.many2one('account.account', 'Refund Tax Account',readonly=True, states={'waiting':[('readonly',False)]},help="Set the account that will be set by default on invoice tax lines for refunds. Leave empty to use the expense account."),
		'account_analytic_collected_id':fields.many2one('account.analytic.account', 'Invoice Tax Analytic Account', readonly=True, states={'waiting':[('readonly',False)]},help="Set the analytic account that will be used by default on the invoice tax lines for invoices. Leave empty if you don't want to use an analytic account on the invoice tax lines by default."),
		'account_analytic_paid_id':fields.many2one('account.analytic.account', 'Refund Tax Analytic Account',readonly=True, states={'waiting':[('readonly',False)]}, help="Set the analytic account that will be used by default on the invoice tax lines for refunds. Leave empty if you don't want to use an analytic account on the invoice tax lines by default."),
		'parent_id':fields.many2one('account.tax', 'Parent Tax Account', select=True,readonly=True, states={'waiting':[('readonly',False)]}),
		'child_ids':fields.one2many('account.tax', 'parent_id', 'Child Tax Accounts',readonly=True, states={'waiting':[('readonly',False)]}),
		'child_depend':fields.boolean('Tax on Children', readonly=True, states={'waiting':[('readonly',False)]},help="Set if the tax computation is based on the computation of child taxes rather than on the total amount."),
		'python_compute':fields.text('Python Code',readonly=True, states={'waiting':[('readonly',False)]}),
		'python_compute_inv':fields.text('Python Code (reverse)',readonly=True, states={'waiting':[('readonly',False)]}),
		'python_applicable':fields.text('Python Code',readonly=True, states={'waiting':[('readonly',False)]}),
		...
		'include_base_amount': fields.boolean('Included in base amount',readonly=True, states={'waiting':[('readonly',False)]},help="Indicates if the amount of tax must be included in the base amount for the computation of the next taxes"),
		'company_id': fields.many2one('res.company', 'Company', required=True),
		'description': fields.char('Tax Code',readonly=True, states={'waiting':[('readonly',False)]}),
		'price_include': fields.boolean('Tax Included in Price', readonly=True, states={'waiting':[('readonly',False)]},help="Check this if the price you use on the product and invoices includes this tax."),
		'type_tax_use': fields.selection([('sale','Sale'),('purchase','Purchase'),('all','All'),('service','Service'),('ed','ED')], 'Tax Application', required=True,readonly=True, states={'waiting':[('readonly',False)]}),
	...
	def edit_tax(self, cr, uid, ids, context=None):
		return self.write(cr, uid, ids, {'state':'waiting'})	
	...
	def _unit_compute(self, cr, uid, taxes, price_unit, product=None, partner=None, quantity=0):
		...
		for tax in taxes:
			...
			print "amount2 -------22222222222----------------",amount2 
			if tax.child_ids:
				if tax.child_depend:
					latest = res.pop()
				amount = amount2
				child_tax = self._unit_compute(cr, uid, tax.child_ids, amount, product, partner, quantity)
				print "child_tax -------child_tax ------------", child_tax
				b = [d['amount'] for d in child_tax if 'amount' in d]
				print "bbbbbbbbbbbbbbbbbbbbbbbbbbbbb",b
				k = b[0]
				test = reduce(lambda x, y: x + y, b)
				print "aaaaaaaaaaaaaaaaaaaaaaa------------", test
				"""				
				if len(b) != 1:
					for i in b:
						k += i
					...
					k = b[0]
					"""
				print "SSSSSSSSSSSSS -------- SSSSSSSSSS---------",k
			...
			if tax.include_base_amount:
				...
				print "adddddddddddd ----------cur_price_unit:::", cur_price_unit
				print "adddddddddddd ----------b:::", k
				cur_price_unit = cur_price_unit + k
				if tax.child_ids:
					print "tax.child_ids ----------- tax.child_ids----", tax.child_ids
				print "cur_price_unit ---------dddddddd-------------",cur_price_unit
		return res
	...
	def _unit_compute_inv(self, cr, uid, taxes, price_unit, product=None, partner=None):
		print "_unit_compute_inv --- _unit_compute_inv called ^^^^^^^^^^^^^^^^^^"
		taxes = self._applicable(cr, uid, taxes, price_unit,  product, partner)
		res = []
		taxes.reverse()
		cur_price_unit = price_unit
		...
		for tax in taxes:
			#print "TAX :::::::::::::::: ============>>>", tax
			#print "tax----------------child-------child---------", tax.child_ids
			if tax.type=='percent':
				if tax.include_base_amount:
					amount = cur_price_unit - (cur_price_unit / (1 + tax.amount))
					print "amount ----------fffff--IFFFF----------------->>", amount
				else:
					amount = (cur_price_unit / (1 + tax_parent_tot)) * tax.amount
					print "amount -------iiiii-----else----------------->>", amount
			...
			if tax.include_base_amount:
				print "cur_price_unit ---------jjjjjjjjj--------before----", cur_price_unit
				cur_price_unit -= amount
				print "cur_price_unit ----------kkkkkkkkk-------after----", cur_price_unit
				todo = 0
			else:
				todo = 1
			res.append({
	...
	def compute_inv(self, cr, uid, taxes, price_unit, quantity, product=None, partner=None, precision=None):
		print "compute_inv --- compute_inv called ^^^^^^^^^^^^^^^^^^"
		"""
		Compute tax values for given PRICE_UNIT, QUANTITY and a buyer/seller ADDRESS_ID.
