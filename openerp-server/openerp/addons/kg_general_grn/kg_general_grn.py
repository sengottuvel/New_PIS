...
class kg_general_grn(osv.osv):
	...
	def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
		...
		for order in self.browse(cr, uid, ids, context=context):
			...
			for line in order.grn_line:
				...
				line_total += line.grn_qty * line.price_unit
			...
			res[order.id]['amount_total']=(round(val1 + res[order.id]['other_charge'],0))
	...
	_columns = {
		...
		'purpose': fields.selection([('consu', 'Consumables'), ('project', 'Project'), ('asset', 'Asset')], 'Purpose', states={'done':[('readonly',True)]}),
	...
	def kg_grn_confirm(self, cr, uid, ids,context=None):
		...
		for line in grn_entry.grn_line:
			...
			if line.exp_batch_id:
				...
				if exp_grn_qty < line.grn_qty:
					raise osv.except_osv(_('Please Check!'), _('Quantity should not less than GRN Quantity !!'))
	...
	def kg_update_stock_move(self, cr, uid, ids,context=None):
		grn_entry = self.browse(cr, uid, ids[0])
		product_obj = self.pool.get('product.product')
		move_obj = self.pool.get('stock.move')
		product_search = product_obj.search(cr,uid,[('active','=',True),('state','=','approved'),('categ_id','!=',1)])
		for product in product_search:
			product_browse = product_obj.browse(cr,uid,product)
			cat_rec = self.pool.get('product.category').browse(cr,uid,product_browse.categ_id.id)
			category = cat_rec.complete_name
			cat = category.split("/")
			product_cat = product_browse.categ_id.id
			print "---------------id------------------------------",product_browse.name_template
			print "--------------product_cat----------------------------",product_cat
			dummy = [False,False,False,False,False,False]
			
			for item in cat:
				dummy_cat = category
				dummy_split = dummy_cat.split("/")
				
				dummy_join = '/'.join(dummy_split)
				
				
				
				if  dummy_split:
					
					cate_search = self.pool.get('product.category').search(cr,uid,[('id','=',product_cat)])
					
					cate_browse = self.pool.get('product.category').browse(cr,uid,cate_search[0])
					print "---------------id------------------------------",cate_browse.complete_name
					print "---------------id------------------------------",cate_search
					if  len(dummy_split) == 6:
							dummy[5] = cate_search[0]
					if  len(dummy_split) == 5:
							dummy[4] = cate_search[0]
					if  len(dummy_split) == 4:
							dummy[3] = cate_search[0]
					if  len(dummy_split) == 3:
							dummy[2] = cate_search[0]
					if  len(dummy_split) == 2:
							dummy[1] = cate_search[0]
					if  len(dummy_split) == 1:
							dummy[0] = cate_search[0]
				...
				if cate_browse.parent_id:
					product_cat = cate_browse.parent_id.id
				else:
					pass	
				
				category = '/'.join(dummy_split)
				
			move_search = move_obj.search(cr,uid,[('product_id','=',product_browse.id)])
			if move_search:
				for item in move_search:
					move_obj.write(cr,uid,item,{'categ_id':product_browse.categ_id.id,
					'categ1':dummy[0],
					'categ2':dummy[1],
					'categ3':dummy[2],
					'categ4':dummy[3],
	...
	def kg_grn_approve(self, cr, uid, ids,context=None):
		...
		if grn_entry.confirmed_by.id == uid:
			...
			for line in grn_entry.grn_line:
				...
				delete_search = lot_obj.search(cr,uid,[('lot_type','=','in'),('brand_id','=',line.brand_id.id),('product_id','=',line.product_id.id),('price_unit','=',line.price_unit),('grn_no','=',line.grn_id.name),('batch_no','=',line.grn_id.name)])
				
				
				
				if delete_search:
					del_sql1 = """delete from stock_production_lot where id="""+str(delete_search[0])+""" """
					cr.execute(del_sql1)

				print "------------------------hai----------------------------",(line.price_subtotal / line.grn_qty)
				...
				stock_move_obj.create(cr,uid,
					...
					'price_tax':(line.price_subtotal / line.grn_qty) or 0.0,
					})
				if grn_entry.grn_dc == 'dc_invoice' and grn_entry.bill == 'applicable':
...
class kg_general_grn_line(osv.osv):
	...
	def _amount_line(self, cr, uid, ids, prop, arg, context=None):
		...
		for line in self.browse(cr, uid, ids, context=context):
			...
			res[line.id] = (round(taxes['total_included'],2))
		return res
