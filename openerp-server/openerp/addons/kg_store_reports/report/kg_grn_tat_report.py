...
class kg_grn_tat_report(JasperDataParser.JasperDataParser):
	def __init__(self, cr, uid, ids, data, context):
		super(kg_grn_tat_report, self).__init__(cr, uid, ids, data, context)
	...
	#	account_rec = data['form']['account_id']
		...
		if data['form']['status']:
			...
			if status == 'done':
				val['status'] = 'done'
				val['p_status'] = 'Approved'
			if status == 'inv':	
				val['status'] = 'inv'
				val['p_status'] = 'Invoiced'
		else:
		...
		if data['form']['payment_type']:
			...
			if payment_type == 'cash':
				...
				val['p_payment_type'] = 'Cash'
			if payment_type == 'credit':	
				val['payment_type'] = 'credit'
				val['p_payment_type'] = 'Credit'
		else:
			
			val['p_payment_type'] = 'All'	
...
jasper_report.report_jasper('report.jasper_kg_grn_tat_report', 'res.users', parser=kg_grn_tat_report)
