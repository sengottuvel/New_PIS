...
class closing_stock_report(report_sxw.rml_parse):
	...
	def get_data(self,form):
		...
		for prod in product_list:
			...
			out_sql = """ select product_id,sum(product_qty) as qty from stock_move where product_id=%s and move_type='out' and state='done' and location_dest_id = %s and date::date <= '%s' group by product_id """%(prod,location[0],form['date'])
			self.cr.execute(out_sql)			
			out_data = self.cr.dictfetchall()
			
			out_sql1 = """ select product_id,sum(product_qty) as qty from stock_move where product_id=%s and move_type='cons' and state='done' and location_dest_id = %s and date::date <= '%s' group by product_id """%(prod,loc[0],form['date'])
			self.cr.execute(out_sql1)			
			out_data1 = self.cr.dictfetchall()
			
			
			price_sql = """ select avg(price_unit) as price from stock_move where product_id=%s and move_type='out' and state='done' and price_unit > 0 """%(prod)
			self.cr.execute(price_sql)			
			price_data = self.cr.dictfetchall()
				...
				if diff > 0 :
					...
					if price_data:
						if price_data[0]['price'] > 0:
							ans['unit_price'] = price_data[0]['price']
						else:
							ans['unit_price'] = 0.00	
					else:
						ans['unit_price'] = 0.00
					...
					ans['closing_value'] = diff * ans['unit_price']
