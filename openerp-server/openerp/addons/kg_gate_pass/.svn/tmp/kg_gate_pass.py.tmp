...
class kg_gate_pass(osv.osv):
	...
	def confirm_entry(self, cr, uid, ids, context=None):	
		...
		if not entry.name:
			pass_no = ''
			pass_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.gate.pass')])
			rec1 = self.pool.get('ir.sequence').browse(cr,uid,pass_no_id[0])
			cr.execute("""select generatesequenceno(%s,'%s','%s') """%(pass_no_id[0],rec1.code,entry.date))
			pass_number = cr.fetchone();
			pass_no = pass_number[0]
