...
class kg_issue_return(osv.osv):
	...
	def confirm_issue_return(self, cr, uid, ids,context=None):
		...
		if not rec.name:
			...
			issue_no = self.pool.get('ir.sequence').get(cr, uid, 'kg.issue.return') or ''
			issue_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.issue.return')])
			rec1 = self.pool.get('ir.sequence').browse(cr,uid,issue_no_id[0])
			print "--------------------------->",rec
			cr.execute("""select generatesequenceno(%s,'%s','%s') """%(issue_no_id[0],rec1.code,rec.date))
			issue_name = cr.fetchone();	
			rec.write({'name': str(issue_name[0])})
