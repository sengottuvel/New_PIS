...
class kg_product(osv.osv):
    ...
    _order = "state desc,name_template asc"
    _columns = {
    ...
    def entry_confirm(self,cr,uid,ids,context=None):
        ...
        cr.execute("""select name_template from product_product where id = %d"""%(ids[0]))
        r=cr.fetchall();
        tuple = r[0]
        ...
        if (not vals['email_to']) or (not vals['email_cc']):
            ...
            msg = ir_mail_server.build_email(
                    ...
                    subject = "The product %s is waiting for approval"%tuple[0],
                    body = data[0][0],
    ...
    def entry_approve(self,cr,uid,ids,context=None):
        ...
        if (not vals['email_to']) or (not vals['email_cc']):
            ...
            msg = ir_mail_server.build_email(
                    email_from = vals['email_from'][0],
                    email_to = vals['email_to'],
                    subject = " New Product(%s) - Approved"%tuple[0],
                    body = data[0][0],
                    email_cc = vals['email_cc'],
                    object_id = ids[0] and ('%s-%s' % (ids[0], 'product.product')),
                    subtype = 'html',
