	...
	def entry_confirm(self,cr,uid,ids,context=None):		
		...
		for line in obj.order_line:
			...
			if line.product_id.type != 'service':
				...
				elif line.stock_qty <=0:
					raise osv.except_osv(
						_('Warning'),
						_('There is no stock for this Product'))		
...
class sale_order_line(osv.osv):
	...
	def create(self, cr, uid, vals, context=None):
		...
		if not out_data and product_rec.type != 'service':
	...
	def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
		...
		sell_obj = self.pool.get('kg.selling.cost.details.line')
		...
		if not out_data and product_obj.type != 'service':
			raise osv.except_osv(_('Warning!'),
							_('There is no Stock for this Product'))
		...
		sell_search = sell_obj.search(cr, uid, [('state','=','approved'),('product_id','=',product)])
		
				
		if sell_search:
			sell_browse = sell_obj.browse(cr, uid, sell_search[0])
			result['price_unit'] = sell_browse.sale_price	
