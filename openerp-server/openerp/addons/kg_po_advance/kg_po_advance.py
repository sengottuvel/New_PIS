...
class kg_po_advance(osv.osv):
	...
	_columns = {
		...
		'po_id':fields.many2one('purchase.order','PO No',
				domain="[('partner_id','=',supplier_id), '&', ('state','!=','draft'),'&',('payment_mode.term_category','=','advance')]",required=True,
		...
		'payment_mode': fields.many2one('kg.payment.master', 'Mode of Payment', required=True,readonly=False, states={'approved':[('readonly',True)]}),
	...
	def onchange_po_id(self, cr, uid, ids, po_id,advance_amt):
		value = {'po_date':'','net_amt':0.00,'payment_mode':'','discount':0.00,'advance_amt':0.00}
		adv_amt = 0.00
		bal_amt = 0.00
		net_amt = 0.00
		...
		return {'value': {
			...
			'payment_mode':po_rec.payment_mode.id,
			'discount':po_rec.payment_mode.discount,
			'advance_amt':(po_rec.payment_mode.discount * po_rec.amount_total)/100,
	...
	def entry_confirm(self, cr, uid, ids,context=None):
		...
		if not advance_rec.name:
			...
			poa_no = self.pool.get('ir.sequence').get(cr, uid, 'kg.po.advance') or ''
			poa_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.po.advance')])
