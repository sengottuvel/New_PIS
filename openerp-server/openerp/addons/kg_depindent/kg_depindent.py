	...
	def confirm_indent(self, cr, uid, ids,context=None):
		...
		for t in self.browse(cr,uid,ids):
			...
			if not indent_obj.name:
				...
				indent_no = self.pool.get('ir.sequence').get(cr, uid, 'kg.depindent') or ''
				indent_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.depindent')])
				rec = self.pool.get('ir.sequence').browse(cr,uid,indent_no_id[0])
				print "--------------------------->",rec
				cr.execute("""select generatesequenceno(%s,'%s','%s') """%(indent_no_id[0],rec.code,indent_obj.ind_date))
				indent_name = cr.fetchone();	
					
			self.write(cr,uid,ids,{'state':'confirm','confirmed_by':uid,'name' :str(indent_name[0])})
