...
class kg_so_advance(osv.osv):
	...
	def entry_confirm(self, cr, uid, ids,context=None):
		...
		if not advance_rec.name:
			...
			soa_no = self.pool.get('ir.sequence').get(cr, uid, 'kg.so.advance') or ''
			soa_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.so.advance')])
			rec = self.pool.get('ir.sequence').browse(cr,uid,soa_no_id[0])
			
			cr.execute("""select generatesequenceno(%s,'%s','%s') """%(soa_no_id[0],rec.code,advance_rec.advance_date))
			soa_name = cr.fetchone();	
			self.write(cr,uid,ids,{'name':str(soa_name[0])})					
