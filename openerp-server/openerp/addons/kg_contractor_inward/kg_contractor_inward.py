...
class kg_contractor_inward(osv.osv):
	...
	def create(self, cr, uid, vals, context=None):
		...
		if vals.get('name','/')=='/':
			inward_no = ''
			inward_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.contractor.inward')])
			rec1 = self.pool.get('ir.sequence').browse(cr,uid,inward_no_id[0])
			print "--------------------------->",rec
			cr.execute("""select generatesequenceno(%s,'%s','%s') """%(inward_no_id[0],rec1.code,vals.get('inward_date')))
			inward_number = cr.fetchone();
			vals['name'] = inward_number[0]
	...
	def entry_confirm(self,cr,uid,ids,context=None):		
		...
		rec1 = self.pool.get('ir.sequence').browse(cr,uid,inward_no_id[0])
		print "--------------------------->",rec
		cr.execute("""select generatesequenceno(%s,'%s','%s') """%(inward_no_id[0],rec1.code,entry_rec.inward_date))
		inward_number = cr.fetchone();	
		self.write(cr, uid, ids, {
					'state': 'confirm',
					'conf_user_id': uid,
					'confirm_date': dt_time,
					'name' : inward_number[0],
					'orderby_no':order_by,
					})
		return True
