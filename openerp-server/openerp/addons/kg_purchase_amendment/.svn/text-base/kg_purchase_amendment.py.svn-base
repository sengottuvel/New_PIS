...
class kg_purchase_amendment(osv.osv):	
	...
	def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
		...
		for order in self.browse(cr, uid, ids, context=context):
			...
			print "------------------------------------:",order.amendment_line
			for line in order.amendment_line:
				print "--------------------------------manju",line.taxes_id_amend
				tot_discount = line.kg_discount_amend + line.kg_discount_per_value_amend
				val1 += (line.product_qty_amend * line.price_unit_amend)
	...
	def approve_amend(self,cr,uid,ids,context={}):
		...
		for amend_line in amend_obj.amendment_line:
			...
			if amend_line.product_qty < amend_line.product_qty_amend:
				...
				grn_id = self.pool.get('po.grn.line').search(cr, uid, [('po_line_id','=',amend_line.po_line_id.id),('state','=','done')])
				print "-------------------------------------------------------------grn_id---->",grn_id
				print "-------------------------------------------------------------grn_id---->",amend_line.po_line_id.id
			...
			if amend_line.note != amend_line.note_amend:
				po_line_obj.write(cr,uid,po_line_id,{'note': amend_line.note_amend})
			
			print "amend_line.id::::::::::", amend_line.taxes_id
			print "amend_line.id:::taxes_id_amend:::::::", amend_line.taxes_id_amend
			
			po_line_obj.write(cr,uid,po_line_id,{'taxes_id': [(6, 0, [x.id for x in amend_line.taxes_id_amend])]})
				...
				if grn_search:
					...
					for line_amend in amend_obj.amendment_line:
						...
						if grn_line_search:
							...
							stock_lot_search = stock_lot_obj.search(cr,uid,[('po_grn_line_id','=',grn_line_search[0])])
							if stock_lot_search:
								stock_move_browse = stock_move_obj.browse(cr,uid,stock_move_search[0])
								for i in stock_lot_search:
									
									stock_lot_obj.write(cr,uid,i,{'price_unit':line_amend.price_unit_amend,'brand_id':line_amend.brand_id_amend.id or False})
