...
class kg_purchase_invoice(osv.osv):
	...
	_columns = {
		...
		'po_grn_ids': fields.many2many('kg.po.grn', 'purchase_invoice_grn_ids', 'invoice_id','grn_id', 'GRN', delete=False,
			 domain="[('state','=','done'),'&',('supplier_id','=',supplier_id),'&',('grn_type','=',type),'&',('billing_status','=','applicable'),'&',('purpose','=',purpose)]"),
		...
		'product_id': fields.related('pogrn_line_ids','product_id', type='many2one', relation='product.product', string='Product'),
		'product_id_gen': fields.related('gengrn_line_ids','product_id', type='many2one', relation='product.product', string='Product'),
		'product_id_so': fields.related('service_line_ids','product_id', type='many2one', relation='product.product', string='Product'),
	...
	def paid(self,cr,uid,ids,context=None):
		...
		if rr:
			for t in rr:
				print "_____paid______",t[0]
				mast_dtt = self.browse(cr,uid,ids[0])
				today_date = today.strftime('%Y-%m-%d')
				
				calll = mast_dtt.net_amt - t[0]
				if calll == 0:
					...
					self.write(cr, uid, ids[0],{'his_state':'paid','payment_date':today_date})
					
					self.write(cr, uid, ids[0],{'bal_amt':calll})
					return False
				else:
					cr.execute(""" select id from kg_purchase_payment_history as a  where a.header_id = %d """ %(mast_dt))
					rrr = cr.fetchall();
					...
					self.write(cr,uid,ids,{'his_state':'pending'})
					self.write(cr,uid,ids,{'bal_amt':calll})			
	...
	def update_actual_values(self, cr, uid, ids,context=None):
		...
		if invoice_rec.grn_type == 'from_po_grn':
			...
			for line in invoice_rec.pogrn_line_ids:
				...
				po_grn_id = line.po_grn_id.id
				if po_grn_id not in dummy_list:
					
				
					
					dummy_list.append(line.po_grn_id.id)
					if line.po_grn_id.other_charge != None:
						final_other_charges += line.po_grn_id.other_charge 
						
				print "------------dummy_listdummy_list---------out --------------------------------------------------------------------",dummy_list		
				...
				print "------------dummy_listdummy_list---------out --------------------------------------------------------------------",final_other_charges	
				...
				pogrn_invoice_line_obj.write(cr, uid, line.id,{'total_amt': line_total,'net_amt':net_amount,'bal_amt':net_amount})
		if invoice_rec.grn_type == 'from_general_grn':
			for line in invoice_rec.gengrn_line_ids:
				val = 0.00
				gen_grn_id = line.general_grn_id.id
				if gen_grn_id not in dummy_list:
					
				
					
					dummy_list.append(line.general_grn_id.id)
					if line.general_grn_id.other_charge != None:
						final_other_charges += line.general_grn_id.other_charge 
				...
				gengrn_invoice_line_obj.write(cr, uid, line.id,{'total_amt': line_total,'net_amt':net_amount,'bal_amt':net_amount})
		if invoice_rec.grn_type == 'others':
			if invoice_rec.type == 'from_so':
				if invoice_rec.soadvance_line_ids:
					for adv_line in invoice_rec.soadvance_line_ids:
			...
			for line in invoice_rec.service_line_ids:
				...
				if invoice_rec.type == 'from_so':
					...
					so_grn_id = line.so_id.id
					if so_grn_id not in dummy_list:
						
						dummy_list.append(line.so_id.id)
						if line.so_id.other_charge != None:
							final_other_charges += line.so_id.other_charge 
				...
				service_invoice_line_obj.write(cr, uid,line.id, {'total_amt': line_total,'net_amt':net_amount,'bal_amt':net_amount})
				
		self.write(cr, uid, ids[0], {
					'total_amt': round(final_total_amount),
					'discount_amt': round(final_tot_discount),
					'tax_amt':round(final_tax),
					'other_charges_amt':round(final_other_charges),
					'actual_amt' : round(final_net_amt) + round(final_other_charges), 
					'invoice_amt': round(final_net_amt) + round(final_other_charges), 
					'advance_adjusted_amt': round(final_adj_amt) or 0.00, 
					'net_amt': round(final_net_amt) + round(final_other_charges),
					'bal_amt': round(final_net_amt) + round(final_other_charges)})
	...
	def compute_values(self, cr, uid, ids,context=None):
		...
		self.write(cr, uid, ids[0], {
					...
					'tax_amt':round(final_tax),
					'other_charges_amt':round(final_other_charges),
					'actual_amt' : round(final_actual_amount), 
					'invoice_amt': round(final_net_value), 
					'advance_adjusted_amt': round(final_adj_amt) or 0.00, 
					'net_amt': round(final_net_value),
					'bal_amt':round(final_net_value)})
