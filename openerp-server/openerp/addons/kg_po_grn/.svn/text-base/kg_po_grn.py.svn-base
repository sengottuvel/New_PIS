...
class kg_po_grn(osv.osv):
	...
	_columns = {
		...
		'purpose': fields.selection([('consu', 'Consumables'), ('project', 'Project'), ('asset', 'Asset')], 'Purpose', states={'item_load':[('readonly',False)],'draft':[('readonly',False)],'confirmed':[('readonly',False)]}),
		...
		'po_ids':fields.many2many('purchase.order', 'multiple_po', 'grn_id', 'po_id', 'PO Nos',
					domain="[('state','=','approved'), '&', ('order_line.pending_qty','>','0'), '&', ('grn_flag','=',False), '&',('partner_id','=',supplier_id), '&', ('order_line.line_state','!=','cancel'),'&',('purpose1','=',purpose)]",readonly=True, states={'item_load':[('readonly',False)],'draft':[('readonly',False)],'confirmed':[('readonly',False)]}), 
		...
		if grn_entry.confirmed_by.id == uid:
			...
			for line in grn_entry.line_ids:
				...
				if data1:
					...
					cr.execute(del_sql1)
				print "------------------------HAIHAIHAIIII-------------------------------------------->",line.price_subtotal
