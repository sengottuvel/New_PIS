...
class stock_invoice_onshipping(osv.osv_memory):
	...
	def _get_journal_id(self, cr, uid, context=None):
		...
		print "model.................",model
		if not model or 'kg.po.grn' not in model:
			return []
		...
		for pick in browse_picking:
			if not pick.line_ids:
				continue
			src_usage = pick.line_ids[0].location_id.usage
			dest_usage = pick.line_ids[0].location_dest_id.usage
			type = pick.type
	...
	def view_init(self, cr, uid, fields_list, context=None):
		...
		pick_obj = self.pool.get('kg.po.grn')
		count = 0
