...
class stock_location(osv.osv):
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		result = super(stock_location,self).write(cr, uid, ids, vals, context=context)
		return result
	...
	def create(self, cr, uid, vals, context=None):
		...
		if vals.get('name'): 
			v_name = vals['name'].strip() 
			vals['name'] = v_name.capitalize()
		result = super(stock_location,self).create(cr, uid, vals, context=context) 
		return result
...
class stock_picking(osv.osv):
	...
	def draft_force_assign(self, cr, uid, ids, *args):
		print "CCCCCCCCCCCCCCCCCCCCCCCCCOOOOOOOOOOOOOOOOOOOOOOOOOOOOO"
	...
	def draft_validate(self, cr, uid, ids, context=None):
		print "summaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	...
	def _prepare_invoice_group(self, cr, uid, picking, partner, invoice, context=None):
		print "_prepare_invoice_group called from PICKING^^^^^^^^^^^^^^^^^^^^66"
	...
	def _prepare_invoice(self, cr, uid, picking, partner, inv_type, journal_id, context=None):
		print "_prepare_invoice called from PICKING^^^^^^^^^^^^^^^^^^^^66"
	...
	def _prepare_invoice_line(self, cr, uid, group, picking, move_line, invoice_id,
		...
		print "_prepare_invoice_line called from PICKING^^^^^^^^^^^^^^^^^^^^66"
	...
	def action_invoice_create(self, cr, uid, ids, journal_id=False,
		...
		print "action_invoice_create called from PICKING^^^^^^^^^^^^^^^^^^^^66"
	...
	def do_partial(self, cr, uid, ids, partial_datas, context=None):
		print "called do_partial from stock.py 22222222222222222222222222"
			...
			for move in too_few:
				...
				if product_qty != 0:
					defaults = {
							...
							'state': 'assigned',
							'move_dest_id': False,
							'price_unit': move.purchase_line_id.price_subtotal/move.purchase_line_id.product_qty,
							'product_uom': product_uoms[move.id]
					}
					
					print "dddddddddddddddddddddddddddddddddddddddddd",defaults
...
class stock_move(osv.osv):
	...
	def action_confirm(self, cr, uid, ids, context=None):
		...
		print "stock move action called <<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>"
		self.write(cr, uid, ids, {'state': 'confirmed'})
	...
	def do_partial(self, cr, uid, ids, partial_datas, context=None):
		print "called do_partial from stock.py 1111111111111111111111111111111111"
...
class stock_move(osv.osv):
	...
	_columns = {
		...
		'prodlot_ikg.pod': fields.many2one('stock.production.lot', 'Serial Number', states={'done': [('readonly', True)]}, help="Serial number is used to put a serial number on the production", select=True),
		'tracking_id': fields.many2one('stock.tracking', 'Pack', select=True, states={'done': [('readonly', True)]}, help="Logistical shipping unit: pallet, box, pack ..."),
...
class stock_location(osv.osv):
	...
	def unlink(self,cr,uid,ids,context=None):
		...
		for rec in self.browse(cr,uid,ids):	
			cr.execute(select id from stock_move where location_dest_id = %s  %(ids[0]))
			data = cr.dictfetchall()
			if data:
				
				raise osv.except_osv(_('Warning!'),
						_('Data has been already used in transaction, it cannot be deleted.'))
