...
class kg_account_invoice(osv.osv):
	...
	def _amount_all(self, cr, uid, ids, name, args, context=None):
		...
		for invoice in self.browse(cr, uid, ids, context=context):
			...
			for line in invoice.tax_line:
				res[invoice.id]['amount_tax'] += (round(val1,0))		
			res[invoice.id]['other_charge'] = (round(invoice.value1 + invoice.value2,0))
			res[invoice.id]['amount_total'] = (round(res[invoice.id]['amount_tax'] + res[invoice.id]['amount_untaxed'],0))
			res[invoice.id]['amount_total'] += (round(res[invoice.id]['other_charge'],0))
			res[invoice.id]['tot_discount'] = (round(val,0))
		return res
	...
	'grn_id': fields.many2one('kg.po.grn','GRN NO',readonly=True, states={'draft':[('readonly',False)]}),
	'general_grn_id': fields.many2one('kg.general.grn','GRN NO',readonly=True, states={'draft':[('readonly',False)]}),
	...
	def approve_pobill(self, cr, uid, ids,context=None):
		...
		if bill_rec.confirmed_by.id == uid:
