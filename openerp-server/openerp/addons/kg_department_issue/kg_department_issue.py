...
class kg_department_issue(osv.osv):
	...
	_columns = {
		...
		'issue_type': fields.selection([('material', 'Material'), ('service', 'Service')], 'Type'),
		...
		'dep_iss_type':fields.selection([('from_ir','From Item Request'),('direct','Direct')],'Issue Type',
	...
	def confirm_issue(self, cr, uid, ids, context=None):
		...
		if not obj_rec.name:
			...
			issue_no = self.pool.get('ir.sequence').get(cr, uid, 'kg.department.issue') or ''
			issue_no_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.department.issue')])
			rec = self.pool.get('ir.sequence').browse(cr,uid,issue_no_id[0])
			print "--------------------------->",rec
			cr.execute("""select generatesequenceno(%s,'%s','%s') """%(issue_no_id[0],rec.code,obj_rec.issue_date))
			issue_name = cr.fetchone();	
			obj_rec.write({'name': str(issue_name[0])})
		...
		if not obj_rec.issue_line_ids:
			...
			for item in obj_rec.issue_line_ids:
				...
				if not lot_data:
					...
					_('No GRN Entry !!'),
					_('There is no GRN reference for this Issue. You must associate GRN entries for Product %s.' %(dep_issue_line_rec.product_id.name)))
				else:					
					...
					dep_issue_line_rec.write({'price_unit': lot_browse.price_unit or 0.0,'product_uom':dep_issue_line_rec.product_id.uom_id.id
								})											
					for i in val:
					...
					if stock_tot < dep_issue_line_rec.issue_qty:
						...
						_('Associated GRN have less Qty compare to issue Qty for product %s.'%(dep_issue_line_rec.product_id.name)))
					else:
						pass
	...
	def action_process(self, cr, uid, ids, context=None):
		...
		for line_ids in issue_record.issue_line_ids:
			if issue_record.issue_type == 'material':
				if issue_record.dep_iss_type == 'from_ir':
					...
					main_location = issue_record.department_id.main_location.id
			...
			if not lot_data:
				raise osv.except_osv(
				_('No GRN Entry !!'),
				_('There is no GRN reference for this Issue. You must associate GRN entries for Product %s.' %(line_ids.product_id.name)))
			else:
				val = [d['lot_id'] for d in lot_data if 'lot_id' in d]
				tot = 0.0
				for i in val:
