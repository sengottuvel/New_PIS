...
class res_partner(osv.osv, format_address):
	...
	_columns = {
		...
		'sup_state': fields.selection([('draft','Draft'),('confirm','Waiting for approval'),('approved','Approved'),
				('reject','Rejected')],'Status', readonly=True),
		'remark': fields.text('Remarks',readonly=False),
	}
	
	_sql_constraints = [
		('name', 'unique(name)', 'Duplicate Supplier Name not allowed!'),
	]
   ...
   """
	...
	_constraints = [
		...
		(_name_validate, 'Supplier name must be unique !!', ['name']),
	...
	_sql_constraints = [
		...
		('name', 'unique(name)', 'Supplier name must be unique !!'),
	]
...
#   _constraints = [(_check_ean_key, 'Error: Invalid ean code', ['ean13'])]
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		#if vals.get('code'):
			#v_code = vals['code'].strip()
			#vals['code'] = v_code.capitalize()
